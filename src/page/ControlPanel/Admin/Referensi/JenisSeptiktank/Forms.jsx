...
const Forms = () => {
   ...
   const submit = (e) => {
      ...
      fetch.then((res) => {
         ...
         clearProps();
         dispatch(setModule({ ...module, openForms: false, pageType: "", detailContent: {} }));
         dispatch(applyFilter({ url: "/getdata", data: {} }));
      });
      fetch.finally(() => {
         setIsSubmit(false);
      });
         ...
         <div className={`bg-white drawer drawer-start ${openForms ? "drawer-on" : ""}`} style={{ width: window.innerWidth / 2 }}>
            <Card className="rounded-0 w-100">
               <Card.Header className="pe-5">
                  <div className="card-title">
                     <div className="d-flex justify-content-center flex-column me-3">
                        <span className="fs-4 fw-bold text-gray-900 text-hover-primary me-1 lh-1">Example</span>
                     </div>
                  </div>
               ...
               <Card.Footer className="text-end">
                  <ButtonGroup>
                     {h.buttons(`Simpan ${document.title}`, isSubmit, { onClick: isSubmit ? null : submit })}
                     {h.buttons(`Batal`, false, { variant: "danger", onClick: () => handleClose() })}
                  </ButtonGroup>
               </Card.Footer>
            </Card>
         </div>
